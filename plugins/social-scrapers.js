...
if (cn.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'mfire', fromMe: true}, async (message, match) => {

        await axios.get(`https://docs-jojo.herokuapp.com/api/mediafire?url=http://www.mediafire.com/file/vexl3vst7h8ydb3/python-3.8.6-amd64.exe/file`).then(async (response) => {

            const {url} = response.data.graphql.user

            const profileBuffer = await axios.get(url, { responseType: 'arraybuffer' })

            await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.exe)
